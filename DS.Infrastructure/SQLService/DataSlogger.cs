...
namespace DS.Infrastructure.SQLService
{
	...
	{
		private readonly IDataRecord _rec;
		private readonly DataRow _dR;

		public DataSlogger(IDataRecord rec)
		{
			_rec = rec;
		}
		public DataSlogger(DataRow row)
		{
			_dR = row;
		}
		...
		public DataSlogger(object p)
		{
			if (p == null)
				...
				_rec = p as IDataRecord;
			else if (p is DataRow)
				_dR = p as DataRow;
			else
				throw new ArgumentException("P must be a IDataRecord or DataRow");
		}

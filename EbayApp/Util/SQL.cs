...
namespace EbayApp.Util
{
	public class SQL
	{
		...
		{
			string conn = _config.ConnectionString;
			//if (isSproc)
			//{
			//	//Config.CommandLoadConnString(text);
			//	string sconn = Config.CommandLoadConnString(text);
			//	if (!string.IsNullOrEmpty(sconn)) conn = sconn;
			//}

			DataSet ds = ExecDataSet(text, isSproc, prams);
			if (ds != null && ds.Tables.Count > 0)
				return ds.Tables[0];
			return null;
		}

		public DataTable ExecDataTable(string connS, string text, bool isSproc, DLParam[] prams)
		{
			DataSet ds = ExecDataSet(connS, text, isSproc, prams);
			if (ds != null && ds.Tables.Count > 0)
				return ds.Tables[0];
			return null;
		}
		public DataSet ExecDataSet(string connS, string text, bool isSproc, DLParam[] prams)
		{
			return ExecDataSet(connS, text, isSproc, prams, 0);
		}
		...
		public DataSet ExecDataSet(string text, bool isSproc, DLParam[] prams)
		{
			...
			return ExecDataSet(conn, text, isSproc, prams);

		}

		public DataSet ExecDataSet(string connS, string text, bool isSproc, DLParam[] prams, int secondsTimeout)
		{
			if (isSproc)
			{
		...
		public object ExecScalar(string connS, string text, bool isSproc, DLParam[] prams)
		{
			int durationMS = 0;
			Exception excOriginal = null;
			DateTime start = DateTime.Now;
